...
namespace Digital_Library.Service.Helpers
{
	...
	{
		public async Task<(string EncryptedFilePath, byte[] IV, byte[] Tag, byte[] EncryptedDEK)> EncryptFileAsync(
						IFormFile file,
						RSA clientPublicKey,
						string outputFilePath)
		{
			...
			while ((bytesRead = await inputStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
			{
				...
				aesGcm.Encrypt(dek.IV, buffer.AsSpan(0, bytesRead), cipherChunk, tag);
			...
			return (outputFilePath, dek.IV, tag, encryptedDEK);
		}
